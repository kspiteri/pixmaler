...
watch(
  () => props.state.config,
  (config) => { if (!isGm.value) renderPreview(config); },
...
<template>
  <div class="page page--mid lobby">
    ...
    <template v-else>
      <p class="lobby__waiting">Waiting for GM to start…</p>
...
.lobby {
  &__gm      { margin-top: $gap-4; }
  &__start   { margin-top: $gap-4; }
  &__waiting { margin-top: $gap-4; }
