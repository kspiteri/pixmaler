...
import type { ServerMsg, ClientMsg } from "./lib/types";
...
<template>
  ...
  <template v-else-if="route === 'room'">
    ...
    <Voting   v-else-if="state.phase === 'VOTING'"  :gallery="gallery" />
    <Results  v-else-if="state.phase === 'RESULTS'" :results="results" />
  </template>
</template>
